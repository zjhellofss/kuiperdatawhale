...
2. 接下来，你需要右键小鲸鱼图标，选择 **附加 Visual Studio Code** 进入 Docker 环境：
...
3. 进入 Docker 环境后的 VSCode 拥有独立的插件环境，我们需要重新安装必须插件：
...
4. 最后，你需要点击工具图表，选择 `GCC 9.4.0 x86_64-linux-gnu` 版本的编译工具，
   ...
   至此，你已经可以尝试编译运行主程序了，只需要选择对应的编译目标（比如图中我选择了 `kuiper_datawhale_course1`），再点击▶ 按钮 即可开始编译。
