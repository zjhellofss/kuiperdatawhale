...
1. 在 VSCode 中，连接容器是一件很简单的事，首先你需要安装 Docker 插件，点击左侧的扩展栏（或是按下 ctrl+shift+X），键入 Docker，随后选择安装即可：
...
2. 此时你只需要右键小鲸鱼图标，选择 **附加 Visual Studio Code** 进入 Docker 环境。
...
3. 接下来我们还需要安装几个插件，在此时的主界面找到扩展，且根据安装 Docker 插件的步骤自行搜索完成以下几款插件的安装：
...
4. 最后，你需要点击工具图表，选择 `GCC 9.4.0 x86_64-linux-gnu` 版本的编译工具，

   至此，你已经可以尝试编译运行主程序了，你只需要选择对应的编译目标（比如图中我选择了 `kuiper_datawhale_course1`），再点击▶ 按钮 即可开始编译。


如果配置成功，程序会出现如下的运行结果，出现`Failed`这是因为本节课作业需要完成一定的代码，如果作业都正确完成，`Failed`标签就会在运行结果中消失。
