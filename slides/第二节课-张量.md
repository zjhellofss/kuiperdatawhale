...
在上一节课中，我们进行了简单的矩阵计算实践；但在深度学习的计算中，我们往往采用的是多种多样的矩阵进行运算，而这些矩阵的集合体通常被称为`张量`。试着构思一个张量应该有哪些成分：维度、为了适应不同的计算和处理方式
...
首先，在所有事情开始前，我们需要创建一个张量。在创建张量——这一多维矩阵的过程中，自然而然想到我们需要用一个`raw_shapes`变量来存储张量的维度，它将决定了`arma::fcube`的具体结构。我们需要根据输入的维度信息创建相应维度的`arma::fcube`，且创建一个用于存储维度的变量。
...
>**思考题**
>
>请你在 test_fill_reshape.cpp 中找到Fill相关的测试并尝试将其改为列优先填充（欢迎你写出自己的单元测试），随后使用`Tensor::Show`接口方法查看这两种结果的区别。
...
值得注意的是，**数据组的数量必须与待填充张量中的元素数量相同**，即`values.size() == total_elems`。（软件工程的习惯很重要）
